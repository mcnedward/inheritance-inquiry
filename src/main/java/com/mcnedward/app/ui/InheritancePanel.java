...
public class InheritancePanel extends JPanel {
    ...
    public void load(JavaSolution solution) {
        mSolution = solution;
        mInheritanceListModel.clear();
//		for (JavaElement javaElement : solution.getAllElements()) {
//			Stack<JavaElement> inheritanceTree = mSolution.findDepthOfInheritanceTreeFor(javaElement);
...
//		Stack<JavaElement> classStack = mSolution.findDepthOfInheritanceTreeFor(selectedClass);
...
//		}
     ...
     *
     * @param mouseAdapter The MouseAdapter defining the action to happen when an item in the list is selected.
